...
/* -----------------------------------------------
 * 表示速度改善(第三段階・PJAX対応)
 * ページ種別ごとに「読み込むべき専用JSファイル」と「そのJSへ渡す
 * インラインconfig（worksAjax等のグローバル変数）」を1箇所で判定する関数です。
 *
 * これまでは template-parts/script.php の中で is_home() などの条件分岐を直接
 * 書いており、初回ページロード時にしか評価されませんでした。
 * PJAXでのページ遷移後も「このページには何のJSが必要か」を判定できるようにするため、
 * この関数を切り出し、
 *   1) 初回ロード時: template-parts/script.php から呼び出して <script> タグを出力
 *   2) PJAX遷移時  : header.php で #pjax-content の data-page-script /
 *                     data-page-config 属性として出力し、
 *                     assets/js/pjax.js が遷移先HTMLから読み取って
 *                     対応するJSファイルを動的に読み込み直す
 * という2箇所で共通利用します。
 *
 * 戻り値:
 *   array(
 *     'script' => '' | 'top' | 'about' | 'dialogue-single' |
 *                 'works-archive' | 'dialogue-archive' | 'dlog-archive',
 *     'config' => null | array( JSに渡すインラインconfig（連想配列） ),
 *     'config_var' => null | 'worksAjax' などのグローバル変数名,
 *   )
 * -------------------------------------------- */
function designd_get_pjax_page_meta() {
	// トップページ
	...
	if ( is_post_type_archive( 'dialogue' ) ) {
		return array(
			'script'     => 'dialogue-archive',
			'config'     => array(
				'ajaxurl' => admin_url( 'admin-ajax.php' ),
			),
			'config_var' => 'dialogueAjax',
		);
	}
	...
	if ( is_post_type_archive( 'dlog' ) ) {
		return array(
			...
			'config'     => array(
				'ajaxurl'    => admin_url( 'admin-ajax.php' ),
				'archiveUrl' => get_post_type_archive_link( 'dlog' ),
			),
			'config_var' => 'dlogAjax',
		);
	}
